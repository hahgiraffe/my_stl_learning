/*
 * @Author: haha_giraffe
 * @Date: 2019-12-12 14:39:08
 * @Description: unordered_map
 */
#ifndef MINISTL_SRC_CONTAINER_ASSOCIATIVE_UNORDERED_MAP_H
#define MINISTL_SRC_CONTAINER_ASSOCIATIVE_UNORDERED_MAP_H
...
public:
    ...
    unordered_map(size_type n, const hasher& hf, const key_equal& eq) : rep(n, hf, eq) { }
    
    template<typename InputIterator>
    unordered_map(InputIterator first, InputIterator last) : rep(100, hasher(), key_equal()) { rep.insert_unique(first, last); }
    template<typename InputIterator>
    unordered_map(InputIterator first, InputIterator last, size_type n) : rep(n, hasher(), key_equal()) { rep.insert_unique(first, last); }
    template<typename InputIterator>
    unordered_map(InputIterator first, InputIterator last, size_type n, const hasher& hf) : rep(n, hf, key_equal()) { rep.insert_unique(first, last); }
    template<typename InputIterator>
    unordered_map(InputIterator first, InputIterator last, size_type n, const hasher& hf, const key_equal& eq) : rep(n, hf, eq) { rep.insert_unique(first, last); }
    
    size_type size() const { return rep.size(); }
    size_type max_size() const { return rep.max_size(); }
    bool empty() const { return rep.empty(); }
    void swap(unordered_map& m) {rep.swap(m.rep); }
    ...
    pair<iterator, bool> insert_noresize(const value_type& v){
        return rep.insert_unique_noresize(v);
    }
...
inline bool operator==(const unordered_map<Key, T, HashFun, EqualKey, Alloc>& lhs,
                        const unordered_map<Key, T, HashFun, EqualKey, Alloc>& rhs){
    return lhs.rep == rhs.rep;
}

}       //namespace MINISTL

#endif // MINISTL_SRC_CONTAINER_ASSOCIATIVE_UNORDERED_MAP_H
